...
/// A Custom trait to convert a `MemoryMapTag` into a `MemoryMap`.
pub trait FromMemoryMapTag {
    /// Converts a `MemoryMapTag` into a `MemoryMap`.
    ///
    /// # Arguments
    /// - `tag`: A reference to the `MemoryMapTag` to be converted.
    ///
    /// # Returns
    /// - A new `MemoryMap` containing the memory regions from the `MemoryMapTag`.
    fn from_memory_map_tag(tag: &MemoryMapTag) -> Self;
}

impl FromMemoryMapTag for MemoryMap {
    fn from_memory_map_tag(tag: &MemoryMapTag) -> Self {
        ...
        for area in tag.memory_areas() {
            let region_type = match area.typ().into() {
                ...
                MemoryAreaType::Custom(_) => MemoryRegionType::NonExhaustive,
            };
